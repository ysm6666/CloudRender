        ...
        Pass
        {
           ...
           // #define PI 3.14159
            ...
            float3 LightMarch(float3 pos)
            {
                ...
                float transmittance=exp(-1*totalDensity*_LightAbsorption);
                ...
                return _DarkThreshold+(1-_DarkThreshold)*transmittance*cloudColor;
            }
            ...
            float DualHg(float cosTheta,float g1,float g2,float lerpFactor)
            {
            ...
            float4 Frag(Varyings input):SV_Target
            {
                ...
                // 米氏散射
                float phaseVal=DualHg(cosTheta,_PhaseParams.x,_PhaseParams.y,_PhaseParams.z);
                ...
                for (int i=0;i<_MaxStepCount;i++)
                {
                    ...
                    if (density>0)
                    {
                        float3 lightCol=LightMarch(currentPos);
                ...
                //return float4(translength,translength,translength,1);
            }
